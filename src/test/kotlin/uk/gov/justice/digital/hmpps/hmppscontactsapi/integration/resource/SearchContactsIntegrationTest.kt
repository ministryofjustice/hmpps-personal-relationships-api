...
class SearchContactsIntegrationTest : SecureAPIIntegrationTestBase() {
  ...
  @Test
  fun `should get contacts when searched by historical names from audit table`() {
    // Create a contact with an initial name
    val randomDob = LocalDate.now().minusDays(RandomUtils.secure().randomLong(100, 2000))
    val contactId = doWithTemporaryWritePermission {
      ...
      // Update the contact to create historical records in audit table
      testAPIClient.patchAContact(
        mapOf(
          "lastName" to "UpdatedLastName",
          "firstName" to "UpdatedFirstName",
          "middleNames" to "UpdatedMiddle",
        ),
        "/contact/${contact.id}",
      )
    ...
    // Search by the original (historical) last name
    val resultsByHistoricalLastName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "OriginalLastName")
        .build()
        .toUri(),
    )!!

    with(resultsByHistoricalLastName) {
      assertThat(content).isNotEmpty()
      assertThat(content).anyMatch { it.id == contactId }
      val foundContact = content.first { it.id == contactId }
      // Current name should be the updated one
      assertThat(foundContact.lastName).isEqualTo("UpdatedLastName")
      assertThat(foundContact.firstName).isEqualTo("UpdatedFirstName")
    }

    // Search by the original (historical) first name
    val resultsByHistoricalFirstName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "OriginalLast")
        .queryParam("firstName", "OriginalFirst")
        .build()
        .toUri(),
    )!!

    with(resultsByHistoricalFirstName) {
      assertThat(content).isNotEmpty()
      assertThat(content).anyMatch { it.id == contactId }
    }

    // Search by the original (historical) middle name
    val resultsByHistoricalMiddleName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "OriginalLast")
        .queryParam("middleNames", "OriginalMiddle")
        .build()
        .toUri(),
    )!!

    with(resultsByHistoricalMiddleName) {
      assertThat(content).isNotEmpty()
      assertThat(content).anyMatch { it.id == contactId }
    }
  }
  ...
  @Test
  fun `should get contacts when searched by historical names from audit table with soundsLike`() {
    // Create a contact with an initial name that sounds similar to another spelling
    val randomDob = LocalDate.now().minusDays(RandomUtils.secure().randomLong(100, 2000))
    val contactId = doWithTemporaryWritePermission {
      val contact = testAPIClient.createAContact(
        CreateContactRequest(
    ...
    // Search by phonetically similar historical last name (Smith sounds like Smythe)
    val resultsByHistoricalLastName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "Smith")
        .queryParam("soundsLike", true)
    ...
    with(resultsByHistoricalLastName) {
      ...
      val foundContact = content.first { it.id == contactId }
      // Current name should be the updated one
      assertThat(foundContact.lastName).isEqualTo("Johnson")
      assertThat(foundContact.firstName).isEqualTo("Sarah")
    }

    // Search by phonetically similar historical first name (Katharine sounds like Kathryn)
    val resultsByHistoricalFirstName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "Smith")
        .queryParam("firstName", "Katharine")
        .queryParam("soundsLike", true)
        .build()
        .toUri(),
    )!!
    ...
    // Search by phonetically similar historical middle name (Elizabet sounds like Elisabeth)
    val resultsByHistoricalMiddleName = testAPIClient.getSearchContactResults(
      UriComponentsBuilder.fromPath("contact/search")
        .queryParam("lastName", "Smith")
        .queryParam("middleNames", "Elizabet")
        .queryParam("soundsLike", true)
        .build()
        .toUri(),
    )!!
