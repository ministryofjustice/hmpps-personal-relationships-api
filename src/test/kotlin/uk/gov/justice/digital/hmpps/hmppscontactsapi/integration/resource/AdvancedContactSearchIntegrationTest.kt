...
class AdvancedContactSearchIntegrationTest : SecureAPIIntegrationTestBase() {
  ...
  override fun baseRequestBuilder(): WebTestClient.RequestHeadersSpec<*> = webTestClient.get()
    .uri(ADVANCED_CONTACT_SEARCH_URL.toString())
    .accept(MediaType.APPLICATION_JSON)

  @ParameterizedTest
  @CsvSource(
    "contact/advanced-search?lastName=%00%00%27%7C%7C(SELECT%20version())%7C%7C%27,Validation failure: advancedSearchContacts.lastName: must be a letter or punctuation",
    "contact/advanced-search?lastName=foo&middleNames=%00%00%27%7C%7C(SELECT%20version())%7C%7C%27,Validation failure: advancedSearchContacts.middleNames: must be a letter or punctuation",
    "contact/advanced-search?lastName=foo&firstName=%00%00%27%7C%7C(SELECT%20version())%7C%7C%27,Validation failure: advancedSearchContacts.firstName: must be a letter or punctuation",
    "contact/advanced-search?lastName=   &middleNames=foo,Validation failure: advancedSearchContacts.lastName: must not be blank",
    "contact/advanced-search?includeAnyExistingRelationshipsToPrisoner=A-B-1&lastName=foo,Validation failure: advancedSearchContacts.includeAnyExistingRelationshipsToPrisoner: must contain only letters or numbers",
    "contact/advanced-search?dateOfBirth=30/12/2150&lastName=foo,Validation failure: advancedSearchContacts.dateOfBirth: The date of birth must be in the past",
  )
  ...
  fun `should get bad request when searched with empty last name`() {
    val uri = UriComponentsBuilder.fromPath("contact/advanced-search")
      .queryParam("lastName", "")
    ...
    assertThat(errors.developerMessage).isEqualTo("advancedSearchContacts.lastName: must not be blank")
  }

  @Test
  fun `should get bad request when searched with invalid date format for date of birth`() {
    val uri: URI = UriComponentsBuilder.fromPath("contact/advanced-search")
  ...
  fun `should get bad request when searched with no last name`() {
    ...
    assertThat(errors.developerMessage).contains("advancedSearchContacts.lastName: must not be blank")
  }

  @Nested
  inner class PhoneticSearchTests {
    @ParameterizedTest
    @CsvSource(
    ...
    fun `should get the contacts when searched by first name and last name with similar match`(
      ...
      val body = testAPIClient.getAdvancedSearchContactResults(uri)!!
    ...
    @Test
    fun `phonetic search returns empty when no similar matches`() {
      val uri = UriComponentsBuilder.fromPath("contact/advanced-search")
        .queryParam("lastName", "NoSuchLastNameForSoundex")
        .queryParam("firstName", "NoSuchFirstName")
        .queryParam("soundsLike", true)
        .build()
        .toUri()
      ...
      assertThat(body?.page?.totalElements).isEqualTo(0)
    }
  }

  @Nested
  inner class PartialMatchSearchTests {
    @Test
    fun `should return empty list if the contact doesn't exist`() {
      val url = UriComponentsBuilder.fromPath("contact/advanced-search")
      ...
      val body = testAPIClient.getAdvancedSearchContactResults(url)
    ...
    fun `should get the contact with when search by first, middle, last and date of birth`() {
      val body = testAPIClient.getAdvancedSearchContactResults(ADVANCED_CONTACT_SEARCH_URL)
    ...
    fun `should sort by date of birth with nulls as eldest`() {
      ...
      val resultsEldestFirst = testAPIClient.getAdvancedSearchContactResults(
        UriComponentsBuilder.fromPath("contact/advanced-search")
      ...
      val resultsYoungestFirst = testAPIClient.getAdvancedSearchContactResults(
        UriComponentsBuilder.fromPath("contact/advanced-search")
          .queryParam("lastName", randomLastName)
    ...
    @Test
    fun `should sort by names in specified order`() {
      ...
      val ascendingName = testAPIClient.getAdvancedSearchContactResults(
        UriComponentsBuilder.fromPath("contact/advanced-search")
      ...
      val descendingName = testAPIClient.getAdvancedSearchContactResults(
        UriComponentsBuilder.fromPath("contact/advanced-search")
          .queryParam("lastName", "A")
          .queryParam("dateOfBirth", randomDob.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))
    ...
    fun `should return existing relationships to prisoner if requested`() {
      ...
      val results = testAPIClient.getAdvancedSearchContactResults(
        UriComponentsBuilder.fromPath("contact/advanced-search")
    ...
    // extra small test for partial match include-any-relationships when none exist
    @Test
    fun `partial match include relationships returns empty existing relationships when none`() {
      val prisonerNumber = "Z9999ZZ"
      stubPrisonSearchWithResponse(prisonerNumber)
      val uri = UriComponentsBuilder.fromPath("contact/advanced-search")
        .queryParam("lastName", "NoAddress")
        .queryParam("includeAnyExistingRelationshipsToPrisoner", prisonerNumber)
      ...
      body.content.forEach { assertThat(it.existingRelationships).isNotNull }
    }
  }

  companion object {
    private val ADVANCED_CONTACT_SEARCH_URL = UriComponentsBuilder.fromPath("contact/advanced-search")
